...
class BailingMM2ImageProcessor(BaseImageProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if size is not None and (
            ...
            size = {"shortest_edge": 64 * 64, "longest_edge": 32 * 32 * 1280}
    ...
    def preprocess(
        ...
        Args:
            ...
            videos_timestamps_seconds:
                The timestamps (in seconds) for each frame in the videos. Only used for video inputs.
        ...
        if videos is not None:
            ...
            for video_idx, images in enumerate(videos):
                ...
                if videos_timestamps_seconds is not None:
                    cur_video_timestamp_seconds = videos_timestamps_seconds[video_idx]
                    aligned_timestamps_seconds = [
                        (
                            cur_video_timestamp_seconds[itx]
                            + cur_video_timestamp_seconds[
                                itx + self.temporal_patch_size - 1
                            ]
                        )
                        / 2.0
                        for itx in range(
                            0,
                            len(cur_video_timestamp_seconds),
        ...
        if videos is not None and pixel_values_videos is not None and video_timestamps_seconds:
            batch_feature["video_timestamps_seconds"] = video_timestamps_seconds
