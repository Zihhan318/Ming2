...
class StableDiffusion3Pipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    ...
    def __call__(
        ...
        if ref_x is not None:
            ...
            if ref_add_noise:
                bsz = ref_x.shape[0]
                noise = torch.randn_like(ref_x)
                sigmas_add = torch.ones(size=(bsz,), device=ref_x.device).float() * ref_add_noise_ratio
                sigmas_add = sigmas_add.view(-1,1,1,1)
                ref_x = ((1.0 - sigmas_add) * ref_x.float() + sigmas_add * noise.float()).to(ref_x.dtype)
        ...
        # 7. Denoising loop
        # ──────────────────── 参数区 ────────────────────
        do_img_cfg      = image_guidance_scale > 1        # 是否做图像 CFG
        img_cfg         = image_guidance_scale            # 图像 CFG 基准
        cfg_mode        = guidance_scale_mode
        print(f"[Hyper-Param]: \n   do_img_cfg: {do_img_cfg} \n   guidance_scale: {guidance_scale} \n   img_cfg: {img_cfg} \n   cfg_mode: {cfg_mode}") 

        null_txt_embeds   = negative_prompt_embeds  
        if ref_x is not None:
            img_zero  = torch.zeros_like(ref_x)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if ref_x is None:
                    ...
                    ref_x_in = torch.cat([img_zero, ref_x, img_zero, ref_x], dim=0).to(latents.dtype)
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if ref_x is None or (not do_img_cfg):
                        noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                        noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
                    else:
                        # uncond, img_cond, full_cond = noise_pred.chunk(3)
                        uncond, img_cond, text_cond, full_cond = noise_pred.chunk(4)
  
                        if cfg_mode == 1:
                            noise_pred = uncond + img_cfg * (img_cond - uncond) + guidance_scale * (full_cond - img_cond) # M1
                        elif cfg_mode == 2:
                            noise_pred = uncond + img_cfg * (img_cond - uncond) # M2 img
                        elif cfg_mode == 3:
                            ...
                            noise_pred = (uncond
                                +  guidance_scale * (text_cond - uncond)   # 文本增量
                                +  img_cfg        * (img_cond - uncond) # 图像增量
                                )    # M6
