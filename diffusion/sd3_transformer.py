...
class SD3Transformer2DModel(
    ...
    def __init__(
        ...
        self.vae_ref_channel_cat = vae_ref_channel_cat

        self.pos_embed = PatchEmbed(
            height=sample_size,
            width=sample_size,
            patch_size=patch_size,
            in_channels=in_channels * 2 if  self.vae_ref_channel_cat else in_channels,
    ...
    def forward(
        ...
        if ref_hidden_states is not None:
            ...
            if self.ref_pos_embed  is None:
                hidden_states_cat0 = torch.cat([hidden_states, ref_hidden_states], dim=0)
                hidden_states_cat0 = self.pos_embed(hidden_states_cat0)
                hidden_states, ref_hidden_states = hidden_states_cat0.chunk(2, dim=0)
        ...
        if (ref_hidden_states is not None) and (not self.vae_ref_channel_cat):
            assert hidden_states.ndim == 3
            input_image_len = hidden_states.shape[1] // 2
            hidden_states = hidden_states[:, : input_image_len, :]
