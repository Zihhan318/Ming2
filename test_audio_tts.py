...
if __name__ == '__main__':
    ...
    mdl_path = "/heyuan2_12/workspace/qingbei/checkpoints/Ming_Flash_2.0_sft1_merged"
    ...
    for i, ori_text in enumerate(tts_texts):
        for spk in ['DB30', 'lingguang']:  
            ...
            for tts_speech, this_text, text_position, duration in talker.omni_audio_generation(
                ...
                # 22 character -> 4s: 16k * 4 /22 = 2909
                if len(all_wavs) != 0 and torch.cat(all_wavs, dim=-1).shape[1] >= len(ori_text) * 2909:
                    continue
                all_wavs.append(tts_speech)
                if this_text != "":
                    print(this_text, tts_speech.shape, tts_speech.shape[1]/vae.sr)
                    
            if all_wavs:
                waveform = torch.cat(all_wavs, dim=-1)
                soundfile.write(this_outfile, waveform.T.numpy(), vae.sr)
                print(f'save audio:\t{this_outfile}\n{ori_text}\t{vae.sr}', waveform.shape[1]/vae.sr)
            else:
